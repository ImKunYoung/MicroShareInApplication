...
import com.sharein.gateway.config.EmbeddedKafka;
import com.sharein.gateway.config.KafkaSseConsumer;
import com.sharein.gateway.config.KafkaSseProducer;
...
class GatewayserviceKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/gatewayservice-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/gatewayservice-kafka/consume")
