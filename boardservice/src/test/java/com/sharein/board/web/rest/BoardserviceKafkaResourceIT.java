package com.sharein.board.web.rest;
...
import com.sharein.board.IntegrationTest;
import com.sharein.board.config.EmbeddedKafka;
import com.sharein.board.config.KafkaSseConsumer;
import com.sharein.board.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class BoardserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/boardservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/boardservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/boardservice-kafka/unregister"));
                return;
            }
        }
